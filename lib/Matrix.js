...
Matrix.solve = function(A, b){
  var x = linear.solve( A.elements(), b );
  return x;
}

Matrix.Zero = function(n, m){
  var newElements = createElements(n, m, 0);
  return new Matrix(newElements);
}

Matrix.I = function(n){
  var AI = new Matrix(n, n, 0);
...
Matrix.prototype.verticalSum = function(){
  var sums = new Array(this._cols);
  for(let i = 0; i < this._cols; i++){
    sums[i] = 0;
    for(let j = 0; j < this._rows; j++){
      sums[i] += this._elements[j][i];
    }
  }
  return sums;
}
...
Matrix.prototype.inverse = function(){
  ...
  var newElements = linear.invert( this._elements );
  return new Matrix(newElements);
}
